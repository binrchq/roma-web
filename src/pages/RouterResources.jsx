...
const getResourceFields = () => {
  return [
    ...
    { key: 'router_name', label: '路由器名称', type: 'text', required: true, placeholder: '例如: core-router-01', description: '路由器的名称或标识', group: '基础信息' },
    { key: 'web_port', label: 'Web管理端口', type: 'number', placeholder: '80', description: '路由器Web管理界面的端口号，通常为80或443', group: 'Web管理' },
    { key: 'web_username', label: 'Web管理用户名', type: 'text', placeholder: '例如: admin', description: '用于登录Web管理界面的用户名', group: 'Web管理' },
    { key: 'web_password', label: 'Web管理密码', type: 'password', placeholder: '输入密码', description: '用于登录Web管理界面的密码', group: 'Web管理' },
    { key: 'port', label: 'SSH端口', type: 'number', placeholder: '22', description: 'SSH连接端口号，用于命令行管理', group: 'SSH连接' },
    { key: 'ipv4_pub', label: '公网IPv4地址', type: 'text', placeholder: '例如: 203.0.113.1', description: '路由器的公网IPv4地址', group: 'SSH连接' },
    { key: 'ipv4_priv', label: '内网IPv4地址', type: 'text', placeholder: '例如: 192.168.1.1', description: '路由器的内网IPv4地址', group: 'SSH连接' },
    { key: 'ipv6', label: 'IPv6地址', type: 'text', placeholder: '例如: 2001:db8::1', description: '路由器的IPv6地址（可选）', group: 'SSH连接' },
    { key: 'username', label: 'SSH用户名', type: 'text', placeholder: '例如: admin', description: '用于SSH登录的用户名', group: 'SSH连接' },
    { key: 'password', label: 'SSH密码', type: 'password', placeholder: '输入密码', description: '用于SSH登录的密码（与私钥二选一）', group: 'SSH连接' },
    { key: 'private_key', label: 'SSH私钥', type: 'textarea', placeholder: '-----BEGIN RSA PRIVATE KEY-----...', description: 'SSH私钥内容（与密码二选一），支持RSA、ECDSA等格式', group: 'SSH连接' },
    { key: 'description', label: '描述', type: 'text', placeholder: '例如: 核心路由器', description: '资源的描述信息，便于识别和管理', group: '其他信息' },
    { key: 'created_at', label: '创建时间', type: 'datetime', group: '其他信息' },
    { key: 'updated_at', label: '更新时间', type: 'datetime', group: '其他信息' },
  ]
}

export default function RouterResources() {
  ...
  const loadResources = async () => {
    try {
      setLoading(true)
      const data = await api.getResources('router', page, pageSize)
  ...
  const filteredResources = useMemo(() => {
    let filtered = resources.filter((resource) => {
      const nameMatch = nameFilter === "" ||
        (resource.router_name || "").toLowerCase().includes(nameFilter.toLowerCase())
  ...
  const handleView = async (resource) => {
    try {
      const data = await api.getResourceById(resource.id, 'router')
  ...
  const handleDelete = async (resource) => {
    if (confirm(`确定要删除资源 "${resource.router_name}" 吗？`)) {
      try {
        await api.deleteResource(resource.id, 'router')
        await loadResources()
      } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      // 添加空间ID
      const requestData = { ...processedData, type: 'router' }
      if (selectedSpace) {
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">路由器</h1>
          <p className="mt-2 text-sm text-gray-600">管理路由器资源</p>
        </div>
        <Button onClick={() => {
          setFormData({ type: 'router' })
          if (spaces.length > 0) {
            const defaultSpace = spaces.find(s => s.name === 'default')
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>类型: [router]</DialogDescription>
          </DialogHeader>
          {selectedResource && (
