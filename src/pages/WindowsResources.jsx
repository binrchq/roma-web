...
const getResourceFields = () => {
  return [
    ...
    { key: 'hostname', label: '主机名', type: 'text', required: true, placeholder: '例如: win-server-01', description: 'Windows服务器的主机名或标识', group: '基础信息' },
    { key: 'port', label: 'RDP端口', type: 'number', placeholder: '3389', description: '远程桌面协议(RDP)端口号，默认3389', group: '连接信息' },
    { key: 'ipv4_pub', label: '公网IPv4地址', type: 'text', placeholder: '例如: 203.0.113.1', description: '服务器的公网IPv4地址', group: '连接信息' },
    { key: 'ipv4_priv', label: '内网IPv4地址', type: 'text', placeholder: '例如: 192.168.1.100', description: '服务器的内网IPv4地址', group: '连接信息' },
    { key: 'ipv6', label: 'IPv6地址', type: 'text', placeholder: '例如: 2001:db8::1', description: '服务器的IPv6地址（可选）', group: '连接信息' },
    { key: 'port_ipv6', label: 'IPv6 RDP端口', type: 'number', placeholder: '3389', description: 'IPv6连接的RDP端口号', group: '连接信息' },
    { key: 'username', label: 'RDP用户名', type: 'text', placeholder: '例如: Administrator', description: '用于RDP登录的用户名', group: '认证信息' },
    { key: 'password', label: 'RDP密码', type: 'password', placeholder: '输入密码', description: '用于RDP登录的密码', group: '认证信息' },
    { key: 'description', label: '描述', type: 'text', placeholder: '例如: 生产环境Windows服务器', description: '资源的描述信息，便于识别和管理', group: '其他信息' },
    { key: 'created_at', label: '创建时间', type: 'datetime', group: '其他信息' },
    { key: 'updated_at', label: '更新时间', type: 'datetime', group: '其他信息' },
  ]
}

export default function WindowsResources() {
  ...
  const loadResources = async () => {
    try {
      setLoading(true)
      const data = await api.getResources('windows', page, pageSize)
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Windows 服务器</h1>
          <p className="mt-2 text-sm text-gray-600">管理 Windows 服务器资源</p>
        </div>
        <Button onClick={() => {
          setFormData({ type: 'windows' })
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>类型: [windows]</DialogDescription>
          </DialogHeader>
          {selectedResource && (
