...
const getResourceFields = () => {
  return [
    ...
    { key: 'hostname', label: '主机名', type: 'text', required: true, placeholder: '例如: web-server-01', description: 'Linux服务器的主机名或标识', group: '基础信息' },
    ...
    { key: 'description', label: '描述', type: 'text', placeholder: '例如: 生产环境Web服务器', description: '资源的描述信息，便于识别和管理', group: '其他信息' },
    { key: 'created_at', label: '创建时间', type: 'datetime', group: '其他信息' },
    { key: 'updated_at', label: '更新时间', type: 'datetime', group: '其他信息' },
  ]
}

export default function LinuxResources() {
  ...
  const loadResources = async () => {
    try {
      setLoading(true)
      const data = await api.getResources('linux', page, pageSize)
  ...
  const handleView = async (resource) => {
    try {
      ...
      // 为了兼容，如果 data.resource 存在则使用它，否则使用 data 本身
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Linux 服务器</h1>
          <p className="mt-2 text-sm text-gray-600">管理 Linux 服务器资源</p>
        </div>
        <Button onClick={() => {
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>类型: [linux]</DialogDescription>
          </DialogHeader>
          {selectedResource && (
