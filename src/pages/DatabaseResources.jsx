...
const getResourceFields = () => {
  return [
    ...
    { key: 'database_nick', label: '数据库名称', type: 'text', required: true, placeholder: '例如: prod-mysql-01', description: '数据库的标识名称，便于识别', group: '基础信息' },
    { key: 'database_name', label: '数据库名', type: 'text', placeholder: '例如: myapp_db', description: '实际连接的数据库名称', group: '基础信息' },
    { key: 'database_type', label: '数据库类型', type: 'text', placeholder: '例如: MySQL, PostgreSQL, MongoDB', description: '数据库类型，如MySQL、PostgreSQL、MongoDB等', group: '基础信息' },
    { key: 'port', label: '端口', type: 'number', placeholder: '3306', description: '数据库连接端口号（MySQL:3306, PostgreSQL:5432, MongoDB:27017）', group: '连接信息' },
    { key: 'ipv4_pub', label: '公网IPv4地址', type: 'text', placeholder: '例如: 203.0.113.1', description: '数据库服务器的公网IPv4地址', group: '连接信息' },
    { key: 'ipv4_priv', label: '内网IPv4地址', type: 'text', placeholder: '例如: 192.168.1.100', description: '数据库服务器的内网IPv4地址', group: '连接信息' },
    { key: 'ipv6', label: 'IPv6地址', type: 'text', placeholder: '例如: 2001:db8::1', description: '数据库服务器的IPv6地址（可选）', group: '连接信息' },
    { key: 'username', label: '数据库用户名', type: 'text', placeholder: '例如: root', description: '用于连接数据库的用户名', group: '认证信息' },
    { key: 'password', label: '数据库密码', type: 'password', placeholder: '输入密码', description: '用于连接数据库的密码（与私钥二选一）', group: '认证信息' },
    { key: 'private_key', label: '私钥', type: 'textarea', placeholder: '-----BEGIN RSA PRIVATE KEY-----...', description: '数据库认证私钥（与密码二选一），某些数据库支持密钥认证', group: '认证信息' },
    { key: 'description', label: '描述', type: 'text', placeholder: '例如: 生产环境主数据库', description: '资源的描述信息，便于识别和管理', group: '其他信息' },
...
export default function DatabaseResources() {
  ...
  const loadResources = async () => {
    try {
      setLoading(true)
      const data = await api.getResources('database', page, pageSize)
  ...
  const loadDatabaseTypes = async () => {
    try {
      ...
      setDatabaseTypes(list)
      if (!formData.database_type && list.length > 0) {
        setFormData(prev => ({ ...prev, database_type: prev.database_type || list[0].key }))
      }
    } catch (error) {
  ...
  const filteredResources = useMemo(() => {
    let filtered = resources.filter((resource) => {
      const nameMatch = nameFilter === "" ||
        (resource.database_nick || "").toLowerCase().includes(nameFilter.toLowerCase())
  ...
  const handleView = async (resource) => {
    try {
      const data = await api.getResourceById(resource.id, 'database')
  ...
  const handleDelete = async (resource) => {
    if (confirm(`确定要删除资源 "${resource.database_nick}" 吗？`)) {
      try {
        await api.deleteResource(resource.id, 'database')
        await loadResources()
  ...
  const renderField = (field, value) => {
    if (field.key === 'private_key') {
      if (selectedResource && (selectedResource.has_private_key || selectedResource.private_key_masked)) {
        return selectedResource.private_key_masked || '已配置'
      }
      const nested = selectedResource?.resource
      if (nested && (nested.has_private_key || nested.private_key_masked)) {
        return nested.private_key_masked || '已配置'
      }
      return '未配置'
    }
  ...
  const renderFormInput = (field) => {
    if (field.key === 'database_type' && databaseTypes.length > 0) {
      return (
        <select
        ...
        >
          {databaseTypes.map((dbType) => (
            <option key={dbType.key} value={dbType.key}>
              {dbType.label} (默认端口 {dbType.default_port})
            </option>
          ))}
        </select>
      )
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">数据库</h1>
          <p className="mt-2 text-sm text-gray-600">管理数据库资源</p>
        </div>
        <Button onClick={() => {
          setFormData({ type: 'database', database_type: formData.database_type || (databaseTypes[0]?.key || '') })
          if (spaces.length > 0 && !selectedSpace) {
            const defaultSpace = spaces.find(s => s.name === 'default')
            setSelectedSpace((defaultSpace || spaces[0]).id.toString())
          }
          setSelectedRole('')
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>类型: [database]</DialogDescription>
          </DialogHeader>
          {selectedResource && (
